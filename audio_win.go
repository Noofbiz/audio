...
// Rewind rewands the Player
func (p *Player) Rewind() error {
	if p == nil {
		return nil
	}
	al.RewindSources(p.source)
	return lastErr()
}
...
// SetListenerPosition sets the listener position in open AL. X, Y, and Z are in
// the AL coordinates, which are similar to the OpenGL coordinates.
// For example, X would be the X location in pixels divided by the width of the
// game in pixels.
